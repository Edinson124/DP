    ...
    public int[][] calcularBondad(List<Beneficiario> beneDistrito, int preferencial,double[] maxmin, int hombres, int mujeres){
        ...
        for(int i= 0; i< beneDistrito.size(); i++){
            ...
            if(ben.getHorariosRestantes()>0){
                candidatos[cont][0] = i; 
                candidatos[cont][1] = preferencial*( 5*ben.getFlagDis()+ 5*ben.getFlagMayor()) + (1-preferencial) *(5*(1-ben.getFlagDis())+ 5*(1-ben.getFlagMayor()))+1 /1+2*ben.getCantInci() + 3*(((hom*hombres+(1-hom)*mujeres))/(1+(hombres+mujeres))); 
